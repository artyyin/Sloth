...
namespace P1.DAL
{
    ...
    {
        //创建EF框架的上下文
        private DbContext db = EFContentFactory.GetCurrentDbContext();
...
namespace P1.DAL
{
    ...
    public abstract class RepositoryBase<T> : IRepository<T> where T : class
    {
        ...
        private DbContext context = EFContentFactory.GetCurrentDbContext();// new MyConfig().db;
        /// <summary>
        ...
        public virtual int UpdateList<T1>(List<T1> t) where T1 : class
        {
            ...
            try
            {
                foreach (var item in t)
                {
                    this.Context.Entry<T1>(item).State = System.Data.EntityState.Modified;
                }
                return this.Context.SaveChanges();
            }
            catch (Exception e)
